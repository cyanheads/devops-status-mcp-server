/**
 * @fileoverview Tests for the status_suggest_action tool.
 * @module tests/mcp-server/tools/definitions/status-suggest-action.tool.test
 */

import { createMockContext } from '@cyanheads/mcp-ts-core/testing';
import { beforeAll, describe, expect, it } from 'vitest';
import { statusSuggestAction } from '@/mcp-server/tools/definitions/status-suggest-action.tool.js';
...
describe('statusSuggestAction', () => {
  it('returns guidance for a known vendor', () => {
    const ctx = createMockContext();
    const input = statusSuggestAction.input.parse({ vendor: 'github' });
    const result = statusSuggestAction.handler(input, ctx);
  ...
  it('detects category for registered vendors', () => {
    const ctx = createMockContext();
    const input = statusSuggestAction.input.parse({ vendor: 'cloudflare' });
    const result = statusSuggestAction.handler(input, ctx);
  ...
  it('uses DEFAULT_PLAYBOOK for unknown vendor', () => {
    const ctx = createMockContext();
    const input = statusSuggestAction.input.parse({ vendor: 'unknown-vendor-xyz' });
    const result = statusSuggestAction.handler(input, ctx);
  ...
  it('pre-fills domain in suggestions when your_domain is provided', () => {
    const ctx = createMockContext();
    const input = statusSuggestAction.input.parse({
      vendor: 'github',
      your_domain: 'https://api.example.com/path',
    });
    const result = statusSuggestAction.handler(input, ctx);
    // DNS and cert checks should be pre-filled with the domain
    const dnsSuggestion = result.nextToolSuggestions.find((s) => s.toolName === 'status_check_dns');
  ...
  it('formats output with guidance and next steps', () => {
    ...
    const input = statusSuggestAction.input.parse({ vendor: 'github' });
    const result = statusSuggestAction.handler(input, ctx);
    const blocks = statusSuggestAction.format!(result);
    const text = (blocks[0] as { text: string }).text;
    expect(text).toContain('github');
    expect(text).toContain('status_get_incidents');
  });
  ...
  it('raw Statuspage URL as vendor → null category, generic guidance', () => {
    ...
    // status_get_incidents suggestion should still appear with the raw URL as vendor
    const incSuggestion = result.nextToolSuggestions.find(
      (s) => s.toolName === 'status_get_incidents',
    );
  ...
  it('all registered vendor categories produce category-specific guidance', () => {
    ...
    for (const category of categories) {
      ...
      const ctx = createMockContext();
      const input = statusSuggestAction.input.parse({ vendor: slug });
      const result = statusSuggestAction.handler(input, ctx);
