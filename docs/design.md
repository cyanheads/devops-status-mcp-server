...
- `status_check_certs` and `status_check_dns` use only Node.js stdlib (`node:tls`, `node:dns`) — zero additional deps
...
- `status_watch_stack` persists stack configuration via `ctx.state` (tenant-scoped KV)
- `status_suggest_action` outputs guidance and `nextToolSuggestions` pre-filled from incident context — no external calls, fully deterministic
...
Vendor registry is the source of truth for `status_list_vendors`. Any tool accepting a vendor name resolves it by slug (case-insensitive) against the registry first; if no match and the input looks like a URL, it's treated as a raw Statuspage base URL.
...
Returns merged object with `status`, `components`, `incidents`, and `scheduled_maintenances` in a single call. Used by `status_check` in `detailed` mode to minimize round trips.
...
z.object({
  vendors: z.array(z.string().min(1))
    .min(1).max(20)
    .describe('Vendor slugs from the built-in registry (e.g., "github", "cloudflare") or raw Statuspage base URLs (e.g., "https://www.githubstatus.com"). Mix freely. Use status_list_vendors to discover available slugs.'),
...
errors: [
  {
    ...
    recovery: 'Call status_list_vendors to browse available slugs, or pass a full Statuspage base URL (e.g., "https://www.githubstatus.com").',
  },
  {
...
z.object({
  vendor: z.string().min(1)
    .describe('Vendor slug (e.g., "github") or raw Statuspage base URL. Use status_list_vendors to find slugs.'),
...
z.object({
  ...
  vendors: z.array(/* same per-vendor shape as status_check results[] */),
...
errors: [
  ...
  },
  {
    ...
    recovery: 'Call status_list_vendors to find available slugs or pass a full Statuspage base URL.',
  },
]
```
...
**Description:** Instruction tool — returns an incident-response playbook tailored to a vendor degradation, with pre-filled follow-up tool calls. Does not call any external APIs; synthesizes guidance from built-in incident knowledge and the provided context. Use after `status_check` or `status_get_incidents` surfaces a problem to determine what to investigate next. Output includes static mitigation steps specific to the vendor/component category and `nextToolSuggestions` with arguments pre-populated from `affected_components`.
...
z.object({
  ...
  incident_summary: z.string().optional()
    .describe('Latest incident description or update body from status_get_incidents. Paste the most recent update to get more targeted advice.'),
  affected_components: z.array(z.string()).optional()
    .describe('Component names affected (from status_check degraded_components or status_get_incidents affected_components). Tailor suggestions to which subsystem is impacted.'),
...
z.object({
  ...
  nextToolSuggestions: z.array(z.object({
    toolName: z.string().describe('Tool to call next (e.g., "status_check_dns", "status_check_certs").'),
...
|:--------|:------|:--------|
| `statuspage-service` | Atlassian Statuspage public API (`/api/v2/status.json`, `/components.json`, `/incidents.json`, `/scheduled-maintenances.json`, `/summary.json`). In-memory cache (60s TTL keyed by URL). `fetchWithTimeout` + retry via `/utils`. | `status_check`, `status_get_incidents`, `status_watch_stack` |
| `vendor-registry-service` | In-memory registry loaded from `src/data/vendor-registry.ts` at startup. Resolves vendor slugs → Statuspage URLs. Provides category listing and slug→name lookup. | all status tools, `status_list_vendors`, `status_suggest_action` |
| `cert-service` | `node:tls` — direct TLS handshake, no external API. Parses X.509 fields from `tls.DetailedPeerCertificate`. | `status_check_certs` |
| `dns-service` | `node:dns` `Resolver` class — one instance per resolver IP, fanout across record types. | `status_check_dns` |
...
| `STATUS_CACHE_TTL_MS` | No | In-memory cache TTL for Statuspage reads. Default: `60000` (60s). |
| `STATUS_FETCH_TIMEOUT_MS` | No | Per-request timeout for Statuspage API calls. Default: `8000` (8s). |
| `STATUS_CERT_TIMEOUT_MS` | No | Per-domain TLS handshake timeout. Default: `5000` (5s). |
| `STATUS_DNS_TIMEOUT_MS` | No | Per-query DNS timeout. Default: `3000` (3s). |
...
3. **`status_list_vendors`** — first tool, validates the registry shape and slug resolution.
...
5. **`status_check`** — fan-out with `Promise.allSettled`, aggregate health rollup.
6. **`status_get_incidents`** — filter mode (active/resolved/scheduled), duration computation.
7. **`status_watch_stack`** — `ctx.state` read/write for persisted stack, delegates to statuspage-service.
8. **cert-service** — `node:tls` wrapper, `DetailedPeerCertificate` parsing, expiry flagging.
9. **`status_check_certs`** — per-domain `Promise.allSettled`, status classification.
10. **dns-service** — multi-resolver `node:dns.Resolver`, propagation discrepancy detection.
11. **`status_check_dns`** — fan-out across domains × resolvers × record types.
12. **`status_suggest_action`** — static playbook dispatch by `vendor_category`, `nextToolSuggestions` assembly.
...
### Why `status_watch_stack` rather than a polling/subscription model?
...
### Why is there no `status_check_uptime` (HTTP HEAD + latency)?
...
Batch tools (`status_check`, `status_watch_stack`, `status_check_certs`, `status_check_dns`) accept multiple inputs. One failing target should not block the others — the value of a batch check is the full picture. Failed items are surfaced inline with an `error` field. `allSettled` is the correct primitive: `Promise.all` would throw on the first failure and lose all other results.

### Instruction tool vs. LLM sampling

`status_suggest_action` could use `ctx.sample` to ask the client's LLM for dynamic guidance. The risk: non-deterministic output, client dependency, potential latency. The value proposition of this tool is predictable, category-specific playbooks — "Cloudflare CDN is down, here are the known mitigation patterns." Static playbook dispatch by vendor category is deterministic, fast, and works in all clients. If `ctx.sample` is present and the vendor/incident is complex, the handler can optionally enrich the response — but the base path is always static.

### Caching strategy

Statuspage APIs are designed for polling (vendors use them for their own dashboards). 60s TTL is conservative — the official Statuspage dashboard polls more frequently. The TTL is configurable via `STATUS_CACHE_TTL_MS` for users who want fresher data. Cache is in-memory (not `ctx.state`) because it's shared across all tenants — Statuspage data is public and identical for everyone. Cache key: the full Statuspage endpoint URL.
...
- **Vendor self-reporting:** Statuspage data is vendor-published. Vendors may lag incident acknowledgment. `status_check_certs` and `status_check_dns` provide ground-truth checks that complement self-reported status.
- **TLS inspection from server host:** `status_check_certs` connects from wherever the MCP server runs. If the server is hosted, cert checks reflect connectivity from that host — a cert served correctly to the host may still be broken in a specific region. For complete coverage, run the server locally.
- **DNS propagation scope:** `status_check_dns` queries three public resolvers. Propagation completeness across all global resolvers requires a larger resolver set or a dedicated propagation service.
- **`ctx.state` scope:** Stack configuration persisted by `status_watch_stack` is tenant-scoped (per client session in stdio mode, per JWT tenant in HTTP mode). Stack configurations do not persist across server restarts in the default memory storage backend.
